...
int state_my = 0;          // 状态标志
static int value1_my = 0;         // 第一个数值
static int value2_my = 0;         // 第二个数值
static int sign = 1;           // 当前符号
int values[2];          // 存储结果的数组
uint8_t pai_flag;
void parse_char(char c) {
    if(state_my == 0 && c == 'a') {
        // 开始接收第一个数字
        ...
        if(c == '-') {
            ...
            value1_my = value1_my * 10 + (c - '0');  // 累加数字
        }
        else if(c == 'b') {
            value1_my *= sign;   // 应用符号
            state_my = 2;        // 切换到第二个数字
        ...
        if(c == '-') {
            ...
            value2_my = value2_my * 10 + (c - '0');  // 累加数字
        }
        else if(c == 'c') {
            value2_my *= sign;   // 应用符号
            values[0] = value1_my;
