...
uint32_t period_X = 100;
uint32_t period_Y = 100;


// 计算周期值的辅助函数
static uint32_t calculate_period(uint32_t target_freq) {
    #define CLK_FREQ     40000000  // 40MHz
    #define PRESCALE     128       // 固定预分频

    // 计算公式: period = (CLK_FREQ / (PRESCALE * target_freq)) - 1
    uint32_t period = (CLK_FREQ / (PRESCALE * target_freq)) - 1;
    
    // 边界保护 (24位定时器最大值0xFFFFFF)
...
// 定时器A1 (PA15) 频率控制函数
void set_freq_timerA1(uint32_t freq) {
    uint32_t new_period = calculate_period(freq);
    
    // 正确设置周期值的方法：
   DL_Timer_setLoadValue(PWM_1_INST,new_period);// 直接写入LOAD寄存器  
    // 可选：保持原占空比比例
	  uint32_t old_cc = 	DL_Timer_getCaptureCompareValue(PWM_1_INST, GPIO_PWM_1_C0_IDX);
    uint32_t new_cc = (old_cc * new_period) / 100;  // 按比例缩放
		DL_Timer_setCaptureCompareValue(PWM_1_INST,new_cc,GPIO_PWM_1_C0_IDX);
}

// 定时器G7 (PA26) 频率控制函数
void set_freq_timerG7(uint32_t freq) {
    uint32_t new_period = calculate_period(freq);
    
    // 正确设置周期值的方法：
	DL_Timer_setLoadValue(PWM_2_INST,new_period);// 直接写入LOAD寄存器  
	    // 可选：保持原占空比比例
    uint32_t old_cc = 	DL_Timer_getCaptureCompareValue(PWM_2_INST, GPIO_PWM_2_C0_IDX);
    uint32_t new_cc = (old_cc * new_period) / 100;  // 按比例缩放
	DL_Timer_setCaptureCompareValue(PWM_2_INST,new_cc,GPIO_PWM_2_C0_IDX);
}


void set_duty_X(float duty_X)
{
	uint32_t CompareValue_1;
	CompareValue_1 = period_X - period_X*duty_X;
	DL_TimerA_setCaptureCompareValue(PWM_1_INST,CompareValue_1,GPIO_PWM_1_C0_IDX);// 直接写入LOAD寄存器 
	
}	



void set_duty_Y(float duty_Y)
{
	uint32_t CompareValue_2;
	CompareValue_2 = period_Y - period_Y*duty_Y;
	DL_TimerG_setCaptureCompareValue(PWM_2_INST,CompareValue_2,GPIO_PWM_2_C0_IDX);// 直接写入LOAD寄存器 
}	


void set_freq_X(uint32_t freq_X)
{
	period_X = 312500/freq_X;
	DL_TimerA_setLoadValue(PWM_1_INST,period_X);
}

void set_freq_Y(uint32_t freq_Y)
{
	period_Y = 312500/freq_Y;
	DL_TimerG_setLoadValue(PWM_2_INST,period_Y);
}
